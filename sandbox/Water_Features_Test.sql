...
(SELECT MIN (soimoistdept_l) FROM comonth AS co9
INNER JOIN cosoilmoist AS cospt9 ON cospt9.comonthkey=co9.comonthkey AND co9.comonthkey=comonth.comonthkey AND soimoiststat="wet" AND soimoiststat IS NOT NULL)  AS min_soimoistdept_l,
(SELECT MAX (soimoistdept_r) FROM comonth AS co13
INNER JOIN cosoilmoist AS cospt13 ON cospt13.comonthkey=co13.comonthkey AND co13.comonthkey=comonth.comonthkey AND soimoiststat="wet" AND soimoiststat IS NOT NULL)  AS max_soimoistdept_r,
...
(SELECT MAX (soimoistdepb_l) FROM comonth AS co5
...
(SELECT MAX (soimoistdept_r) FROM comonth AS co12
INNER JOIN cosoilmoist AS cospt12 ON cospt12.comonthkey=co12.comonthkey AND co12.comonthkey=comonth.comonthkey  AND ISNULL (soimoiststat, "moist") !="wet")   AS wtbottom_r_moist
FROM legend
...
COLUMN first_month FIRST, last_month LAST, soimoistdept_l_h UNIQUE, wtbottom_l UNIQUE,
soimoistdepb_l_h UNIQUE,
min_soimoistdept_l UNIQUE, 
max_soimoistdept_h UNIQUE,
min_soimoistdepb_l UNIQUE, 
max_soimoistdepb_h UNIQUE, 
wtbottom_r_moist MAX, 
max_soimoistdept_r MAX.
...
#DEFINE soilweb     "http://casoilresource.lawr.ucdavis.edu/soil_web/ssurgo.php?action=explain_mapunit&#38;mukey="||soilwebmukey.
DEFINE soilwebcomp "http://casoilresource.lawr.ucdavis.edu/soil_web/ssurgo.php?action=explain_component&#38;mukey&#61;" ||soilwebmukey || '&#38;cokey=' ||soilwebcokey.

#ASSIGN min_soimoistdepb_l IF min_soimoistdepb_l >= 183 THEN 183 ELSE min_soimoistdepb_l. 
#ASSIGN max_soimoistdepb_h IF max_soimoistdepb_h >= 183 THEN 183 ELSE max_soimoistdepb_h. 
...
DEFINE moisture_b2	#ISNULL(max_soimoistdepb_h) ? "---" : (min_soimoistdepb_l==max_soimoistdepb_h)  ? sprintf("%3.1f", min_soimoistdepb_l/30.48):  sprintf("%3.1f-%-3.1f", min_soimoistdepb_l/30.48, max_soimoistdepb_h/30.48).


IF ISNULL(min_soimoistdepb_l) THEN  "---" ELSE
					...
					#ASSIGN wtbottom_l_moist IF ISNULL(wtbottom_l_moist) THEN 200 ELSE wtbottom_l_moist.

#ASSIGN wtbottom_r_moist ARRAYMAX(wtbottom_r_moist)	.				
#ASSIGN max_soimoistdept_r ARRAYMAX(max_soimoistdept_r)	.	

#DEFINE report_manager_sucks IF wtbottom_r_moist >  max_soimoistdept_r THEN "Perched1"  ELSE "????????!#@".
...
DEFINE apparent		IF ISNULL(max_soimoistdepb_h) THEN "---" ELSE 
					...
					IF wtbottom_l  == max_soimoistdepb_l THEN "Apparent2"  ELSE 
					IF wtbottom_r_moist >  max_soimoistdept_r THEN "Perched1"  ELSE 
					IF wtbottom_l_moist >  min_soimoistdept_l THEN "Perched2" # ELSE 
					
					#IF wtbottom_l  == max_soimoistdepb_l THEN "Apparent2" 
					
					
					
					ELSE "Perched3".
#ASSIGN apparent  ARRAYMAX(apparent).
...
"wtbottom_r_moist",
 "max_soimoistdept_r",
		"wtbottom_l_moist", " min_soimoistdept_l","wtbottom_l", "max_soimoistdepb_l"	
