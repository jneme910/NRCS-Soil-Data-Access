...
INNER JOIN comonth ON comonth.cokey=c_sl.cokey 
INNER JOIN cosoilmoist ON cosoilmoist.comonthkey=comonth.comonthkey 
AND ((ISNULL (soimoiststat, 'Moist') IN ('Dry', 'Moist') AND soimoistdept_r >=60)) --AND (soimoiststat != 'Dry' AND soimoistdept_r >=60)
...
CASE 
WHEN WATER_TABLE_RATING_WET = 'W' AND WATER_TABLE_RATING_DRY = '' THEN 'Wet'
WHEN WATER_TABLE_RATING_WET = 'W' AND WATER_TABLE_RATING_DRY IS NULL THEN 'Wet'
WHEN WATER_TABLE_RATING_WET = 'W' AND WATER_TABLE_RATING_DRY = 'D' THEN 'Perched' ELSE 'Not Rated' END AS perched_apparent
FROM #NRCS590