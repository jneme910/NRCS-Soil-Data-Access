--all fields are based on the representative values of the dominant component (dcp) unless otherwise noted (_dcd) for dominant condition of the map unit
--Begin SQL
SELECT
...
---begin TN Data
, (SELECT interphr FROM component left outer join cointerp ON component.cokey = cointerp.cokey WHERE component.cokey = c.cokey AND ruledepth = 0 AND mrulename like 'Commodity Crop Productivity Index (Corn) (TN)') as TN_NCCPI_Corn_Value_dcp

, (SELECT interphr FROM component left outer join cointerp ON component.cokey = cointerp.cokey WHERE component.cokey = c.cokey AND ruledepth = 0 AND mrulename like 'Commodity Crop Productivity Index (Soybeans) (TN)') as TN_NCCPI_Soy_Value_dcp

, (SELECT interphr FROM component left outer join cointerp ON component.cokey = cointerp.cokey WHERE component.cokey = c.cokey AND ruledepth = 0 AND mrulename like 'ENG - Conventional On-Site Septic Systems (TN)') as TN_Septic_Value_dcp
...
mrulename like 'NCCPI - National Commodity Crop Productivity Index (Ver 2.0)') as NCCPI_Class_dcp
...
mrulename like 'NCCPI - National Commodity Crop Productivity Index (Ver 2.0)') as NCCPI_Value_dcp
, (SELECT interphrc FROM component left outer join cointerp ON component.cokey = cointerp.cokey WHERE component.cokey = c.cokey AND ruledepth = 0 AND 
mrulename like 'NCCPI - NCCPI Corn and Soybeans Submodel (II)') as Nccpi_Corn_Class_dcp 
, (SELECT interphr FROM component left outer join cointerp ON component.cokey = cointerp.cokey WHERE component.cokey = c.cokey AND ruledepth = 0 AND 
mrulename like 'NCCPI - NCCPI Corn and Soybeans Submodel (II)') as Nccpi_Corn_Value_dcp 
, (SELECT interphrc FROM component left outer join cointerp ON component.cokey = cointerp.cokey WHERE component.cokey = c.cokey AND ruledepth = 0 AND 
mrulename like 'NCCPI - NCCPI Small Grains Submodel (II)') as Nccpi_Wheat_Class_dcp
, (SELECT interphr FROM component left outer join cointerp ON component.cokey = cointerp.cokey WHERE component.cokey = c.cokey AND ruledepth = 0 AND 
mrulename like 'NCCPI - NCCPI Small Grains Submodel (II)') as Nccpi_Wheat_Value_dcp
, (SELECT interphrc FROM component left outer join cointerp ON component.cokey = cointerp.cokey WHERE component.cokey = c.cokey AND ruledepth = 0 AND 
mrulename like 'NCCPI - NCCPI Cotton Submodel (II)') as Nccpi_Cotton_Class_dcp
, (SELECT interphr FROM component left outer join cointerp ON component.cokey = cointerp.cokey WHERE component.cokey = c.cokey AND ruledepth = 0 AND 
mrulename like 'NCCPI - NCCPI Cotton Submodel (II)') as Nccpi_Cotton_Value_dcp   
, (SELECT interphrc FROM component left outer join cointerp ON component.cokey = cointerp.cokey WHERE component.cokey = c.cokey AND ruledepth = 0 AND 
...
INNER JOIN mapunit AS mu ON mu.lkey = l.lkey and LEFT(l.areasymbol, 2) like 'TN'
...
, CAST((((Cfact*wei)/tfact)/100) as Decimal(5, 1)) as WindErodIndex
 ,TN_NCCPI_Corn_Value_dcp ,TN_NCCPI_Soy_Value_dcp ,TN_Septic_Value_dcp , NCCPI_Class_dcp
, NCCPI_Value_dcp
, Nccpi_Corn_Class_dcp 
, Nccpi_Corn_Value_dcp 
, Nccpi_Wheat_Class_dcp
, Nccpi_Wheat_Value_dcp
