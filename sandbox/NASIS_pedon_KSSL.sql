...
INNER JOIN [sdmONLINE].[dbo].[lab_pedon] AS p ON p.pedon_key=cnn.pedon_key  --AND upedonid LIKE  '%NE%'
...
LEFT OUTER JOIN [nasis].[dbo].[phsample] AS phs  ON  phs.labsampnum COLLATE SQL_Latin1_General_CP1_CI_AS = l.labsampnum COLLATE SQL_Latin1_General_CP1_CI_AS
...
INNER JOIN (SELECT [area_code] AS ssa_code,[area_name] AS ssa_name, area_key FROM [sdmONLINE].[dbo].lab_area --SSA
 ...
 LEFT OUTER JOIN  [nasis].[dbo].phorizon ON phorizon.phiid=phs.phiidref

ORDER BY p.pedlabsampnum, hzn_top ASC, hzn_bot ASC
