...
AND LEFT (legend.areasymbol,2) = 'OR'
...
INNER JOIN mapunit AS c ON c1.mukey=c.mukey AND c.mukey=#main.mukey ORDER BY c1.comppct_r DESC,CASE WHEN LEFT (muname,2)= LEFT (compname,2) THEN 1 ELSE 2 END ASC, c1.cokey) 
...
--Unit Conversion *100
SELECT DISTINCT
 #main.areasymbol,  #main.areaname,  #main.musym, #main.nationalmusym, #main.muname, #main.mukind,

 #main.mukey, ROUND (SUM (WEIGHTED_CO_SOC_0_30) over(PARTITION BY #SOC4.mukey) ,4) *100  AS SOCSTOCK_0_30 , 
...
Texture, TextCls, ParMatGrp, ParMatKind
FROM #SOC4
RIGHT OUTER JOIN #main ON #main.mukey=#SOC4.mukey
LEFT OUTER JOIN #tex ON #tex.mukey=#main.mukey
