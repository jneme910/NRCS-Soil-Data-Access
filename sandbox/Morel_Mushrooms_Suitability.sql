...
SELECT @area = 'MN'; --Enter State Abbreviation or Soil Survey Area i.e. WI or  WI025,  US 
...
SELECT legend.areaname
     ...
     , (
           ...
           FROM mapunit             AS MM2
               INNER JOIN component AS CCO
                   ON CCO.mukey = MM2.mukey
                      AND mapunit.mukey = MM2.mukey
                      AND (CASE
                               WHEN 1 = @major THEN
                                   0
                               WHEN majcompflag = 'Yes' THEN
                                   0
                               ELSE
                                   1
                           END = 0
     ...
     , (
           SELECT DISTINCT
               SUBSTRING((
                             ...
                             FROM mapunit              AS m
                                 INNER JOIN muaoverlap AS mua
                                     ON mua.mukey = m.mukey
                                 INNER JOIN laoverlap  AS lao
...
SELECT map.mukey
     ...
     , (
           ...
           FROM cogeomordesc
           WHERE c.cokey = cogeomordesc.cokey
     ...
     , CASE
           WHEN taxsubgrp LIKE '%natr%' THEN
               'natr'
           WHEN taxsubgrp LIKE '%gyps%' THEN
               'gyps'
           WHEN taxsubgrp LIKE '%verti%' THEN
               'verti'
           ELSE
               'NA'
       END                                                                AS subgroup
     , CASE
           WHEN taxgrtgroup LIKE '%ert%' THEN
               'verti'
           WHEN taxgrtgroup LIKE '%natr%' THEN
               'natr'
           WHEN taxgrtgroup LIKE '%calci%' THEN
               'calci'
           WHEN taxgrtgroup LIKE '%gyps%' THEN
               'gyps'
           ELSE
               'NA'
       END                                                                AS greatgroup
     ...
     , (
           ...
           FROM chorizon                AS ch2
               INNER JOIN chdesgnsuffix AS chs
     ...
     , (
           ...
           FROM component             AS c2
               INNER JOIN cosurffrags AS cosf
     ...
     , (
           SELECT CASE
                      WHEN MIN(resdept_r) IS NULL THEN
                          200
                      ELSE
                          CAST(MIN(resdept_r) AS INT)
                  END
           FROM component
               LEFT OUTER JOIN corestrictions
     ...
     , (
           ...
           WHERE component.cokey = c.cokey
                 AND reskind IS NOT NULL
                 AND reskind != 'Fragipan'
       )                                                                  AS PD_Fragi
     , (
           SELECT TOP 1
               taxmoistcl
           FROM component AS t
               INNER JOIN cotaxmoistcl
     ...
     , (
           SELECT TOP 1
               CASE
                   WHEN charindex(' ', pmgroupname, charindex(' ', pmgroupname) - 1) = 0 then
                       pmgroupname
                   else
                       left(pmgroupname, charindex(' ', pmgroupname, charindex(' ', pmgroupname) - 1))
               END AS pm
           FROM copmgrp
           WHERE copmgrp.cokey = c.cokey
       )                                                                  AS pmgroupname
FROM #map                AS map
...
SELECT #map.mukey
     ...
     , month
     , c3.cokey
     , MIN(soimoistdept_r) over (partition by c3.cokey) as min_water
FROM #map
    INNER JOIN component  AS c3
...
SELECT DISTINCT
  ...
  , compname
  , (
        SELECT AVG(min_water)
        FROM #water AS w2
        WHERE w2.cokey = #water.cokey
              AND CASE
                      ...
                      WHEN month = 'July' THEN
                          1
                      WHEN month = 'August' THEN
                          1
                      WHEN month = 'September' THEN
                          1
                      ElSE
                          2
                  END = 1
  ...
  , (
        SELECT AVG(min_water)
        FROM #water AS w3
        WHERE w3.cokey = #water.cokey
              AND CASE
                      WHEN month = 'October' THEN
                          1
                      WHEN month = 'November' THEN
                          1
                      WHEN month = 'December' THEN
                          1
                      WHEN month = 'January' THEN
                          1
                      WHEN month = 'February' THEN
                          1
...
SELECT mukey
     ...
     , hzdepb_r
     , CASE
           WHEN (hzdepb_r - hzdept_r) IS NULL THEN
               0
           ELSE
               CAST((hzdepb_r - hzdept_r) AS INT)
       END     AS thickness
     , CASE
           when awc_r IS NULL THEN
               0
           ELSE
               awc_r
       END     AS awc_r
...
SELECT CASE
           WHEN hzdepb_r <= 150 THEN
               hzdepb_r
           WHEN hzdepb_r > 150
                and hzdept_r < 150 THEN
               150
           ELSE
               0
       END AS InRangeBot
     , CASE
           WHEN hzdept_r < 150 then
               hzdept_r
           ELSE
               0
       END AS InRangeTop
     , CASE
           WHEN hzdepb_r <= 20 THEN
               hzdepb_r
           WHEN hzdepb_r > 20
                and hzdept_r < 20 THEN
               20
           ELSE
               0
       END AS InRangeBot_0_20
     , CASE
           WHEN hzdept_r < 20 then
               hzdept_r
           ELSE
               0
       END AS InRangeTop_0_20
     , CASE
           WHEN hzdepb_r <= 50 THEN
               hzdepb_r
           WHEN hzdepb_r > 50
                and hzdept_r < 50 THEN
               50
           ELSE
               20
     ...
     , CASE
           WHEN hzdepb_r <= 100 THEN
               hzdepb_r
           WHEN hzdepb_r > 100
                and hzdept_r < 100 THEN
               100
           ELSE
               50
...
SELECT #comp.cokey
     ...
     , texture
     , CASE
           WHEN desgnmaster LIKE '%h%' THEN
               'H horizon'
           WHEN desgnmaster LIKE '%O%' THEN
               'Organic'
           WHEN desgnmaster NOT LIKE '%O%' THEN
               'Mineral'
           ELSE
               'NA'
       END                                    AS mineral_des
...
FROM #comp
    INNER JOIN(chorizon
    INNER JOIN chtexturegrp
        ON chorizon.chkey = chtexturegrp.chkey)
        ON #comp.cokey = chorizon.cokey
           AND (
                   ((chorizon.hzdept_r) =
                   (
                       SELECT Min(chorizon.hzdept_r) AS MinOfhzdept_r
                       FROM chorizon
                           INNER JOIN chtexturegrp
                               ON chorizon.chkey = chtexturegrp.chkey
...
SELECT #comp.cokey
     ...
     , (
           SELECT TOP 1
               [ChoiceName]
           FROM chtexture AS cht
              , MetadataDomainMaster dm
              , MetadataDomainDetail dd
              , chtexturemod AS chtm
           WHERE chtm.chtkey = cht.chtkey
     ...
     , (
           ...
           FROM chtexture AS cht
              , MetadataDomainMaster dm
              , MetadataDomainDetail dd
           WHERE chtexturegrp.chtgkey = cht.chtgkey
     ...
     , CASE
           WHEN stratextsflag = 'Yes' THEN
               'stratified'
           WHEN desgnmaster = 'O' THEN
               'organic'
...
SELECT cokey
     ...
     , text_grouping
     , CASE
           WHEN text_grouping = 'organic' THEN
               'organic'
           WHEN text_grouping = 'stratified' THEN
               'stratified'
           WHEN tex_modifier IN ( 'ASHY', 'HYDR', 'MEDL' ) THEN
               'volcanic modifier'
           WHEN tex_modifier IN ( 'GS', 'HB', 'MS', 'WD' ) THEN
               'organic soil material modifier'
           WHEN tex_modifier IN ( 'HO', 'MK', 'PT' ) THEN
               'highly organic mineral material modifier'
           WHEN tex_modifier IN ( 'COP', 'DIA', 'MR' ) THEN
               'limnic material modifier'
           WHEN tex_modifier IN ( 'ART', 'ARTV', 'ARTVX' ) THEN
               'anthropogenic material modifier'
           WHEN tex_modifier = 'CEM' THEN
               'cemented material modifier'
           WHEN tex_modifier = 'GYP' THEN
               'gypsiferous material modifier'
           WHEN tex_modifier = 'PF' THEN
               'permanently frozen material modifier'
           WHEN tex = 'COS' THEN
               'coarse textured'
           WHEN tex = 'S' THEN
               'coarse textured'
...
SELECT cokey
     ...
     , hzdept_r
     , hzdepb_r --tex,  
     , MIN(hzdept_r) over (partition by cokey
                                      , texture_grouping
...
WHERE CASE
          WHEN tex_in_lieu = 'spm' THEN
              1
          WHEN tex_in_lieu = 'hpm' THEN
              1
          WHEN tex_in_lieu = 'mpm' THEN
              1
          WHEN tex_in_lieu IS NULL THEN
              2
          ELSE
              2
      END = 2
...
AS (SELECT cokey
         ...
         , row_num
    From #surface_tex3
    WHERE prev_bottom_depth = hzdept_r
...
SELECT DISTINCT
  ...
  , texture_grouping
  , min_top_depth
  , MAX(hzdepb_r) over (partition by cokey, texture_grouping) as max_bottom_depth --, ROW_NUMBER() OVER(PARTITION BY cokey ORDER BY min_top_depth ASC ) AS row_num 
INTO #surface_final
...
SELECT cokey
     ...
     , min_top_depth
     , max_bottom_depth
     , ROW_NUMBER() OVER (PARTITION BY cokey ORDER BY min_top_depth ASC) AS row_num
INTO #surface_final2
FROM #surface_final
ORDER BY cokey
       , min_top_depth ASC
...
(
    SELECT #comp.cokey
         ...
         , featkind
    FROM #comp
        INNER JOIN codiagfeatures AS dia
            ON dia.cokey = #comp.cokey
) #d
PIVOT
(
    MIN(featdept_r)
    FOR featkind IN ([Argillic horizon], [Albic horizon] --Albic materials --Interfingering of albic materials
...
(
    Select #comp.cokey
         , compname
         , featkind
         , Col = concat('Diag', Row_Number() over (Partition By #comp.cokey Order By [featdept_r] ASC))
    FROM #comp
        INNER JOIN codiagfeatures AS dia
            ON dia.cokey = #comp.cokey
               AND [featdept_r] <= 50
...
(
    SELECT #comp.cokey
         ...
         , reskind
    FROM #comp
        INNER JOIN corestrictions AS res
            ON res.cokey = #comp.cokey
) #r
PIVOT
(
    MIN(resdept_r)
    FOR reskind IN ([Densic bedrock], [Lithic bedrock], [Paralithic bedrock], [Cemented horizon], [Duripan], [Fragipan]
                  , [Manufactured layer], [Petrocalcic], [Petroferric], [Petrogypsic]
                   )
) AS #rest_pivot_table;
...
SELECT c3.cokey
     ...
     , CASE
           ...
           WHEN (fragsize_r
               ...
               'gravel'
           WHEN fraghard = 'Noncemented' THEN
               'para'
           ELSE
               'unspecified'
        ...
        ON c3.cokey = ch.cokey
           AND ch.chkey IN (
                               SELECT TOP 1
                                   ch2.chkey
                               FROM chorizon          AS ch2
                                   INNER JOIN chfrags AS chf2
                                       ON chf2.chkey = ch2.chkey
                                          AND c3.cokey = ch2.cokey
                                          AND hzdept_r < 50
                               ORDER BY SUM(fragvol_r) OVER (PARTITION BY ch2.chkey) DESC
                                      , ch2.hzdepb_r ASC
                                      , ch2.chkey
                           )
...
SELECT cokey
     ...
     , [para]
     , [channers and flagstones]
     , total_frags = ISNULL([gravel], 0) + ISNULL([cobbles], 0) + ISNULL([stones and boulders], 0) + ISNULL([para], 0)
                     + ISNULL([channers and flagstones], 0)
FROM
(
    SELECT cokey
         , compname
...
(
    ...
    FOR fragment_class IN ([gravel], [cobbles], [stones and boulders], [para], [channers and flagstones])
) AS #frag_pivot_table;
...
SELECT DISTINCT
  ...
  , CASE
        WHEN sandtotal_r
             BETWEEN 30 AND 80 THEN
            1
        ELSE
            0
    END                              AS ck_sand_30_80
  , CASE
        WHEN silttotal_r
        ...
        ELSE
            0
    END                              AS ck_silt_30_50
  , CASE
        WHEN claytotal_r
             BETWEEN 8 AND 25 THEN
            1
        ELSE
            0
    END                              AS ck_clay_8_25
  , CASE
        WHEN om_r
             BETWEEN 2.8 AND 5 THEN
            1
        ELSE
            0
    END                              AS ck_om_2p8_5
  , CASE
        WHEN maxph1to1h2o_0_15cm
             BETWEEN 6 AND 7.5 THEN
            1
        ELSE
            0
    END                              AS ck_ph_6_7p5
  , CASE
        WHEN drainagecl = 'Well drained' THEN
            1
        ELSE
            0
    END                              AS ck_well_drained
  , #comp.landform
  ...
  , #aws150.aws_50_100cm
  , CASE
        WHEN compkind = 'Miscellaneous area' THEN
            NULL
        WHEN #spd.minsoil_profile_depth IS NULL THEN
            200
        WHEN #spd.minsoil_profile_depth > 200 THEN
            200
        ELSE
            #spd.minsoil_profile_depth
    END                              AS PD_Fragi                  --Switched the names around
