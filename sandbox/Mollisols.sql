...
SELECT @area= 'TX'; --Enter State Abbreviation or Soil Survey Area i.e. WI or WI025
...
esd_id VARCHAR (30), esd_name VARCHAR (254), sum_fragcov_low REAL, sum_fragcov_rv REAL , sum_fragcov_high REAL, major_mu_pct_sum SMALLINT, adj_comp_pct REAL, restrictiodepth SMALLINT, taxclname VARCHAR (120) )
...
slope_r, hydgrp, esd_id , esd_name, sum_fragcov_low, sum_fragcov_rv, sum_fragcov_high, major_mu_pct_sum, adj_comp_pct, restrictiodepth, taxclname )
...
FROM #map AS map
INNER JOIN component AS c ON c.mukey=map.mukey AND majcompflag = 'Yes' AND taxorder = 'mollisols'
-----------Dominant Comoonent
...
#comp.major_mu_pct_sum, #comp.adj_comp_pct, restrictiodepth, taxclname ,
 --#comp 
...
#comp.esd_id, #comp.esd_name, sum_fragcov_low , sum_fragcov_rv, sum_fragcov_high, #comp.major_mu_pct_sum, #comp.adj_comp_pct ,restrictiodepth, taxclname ,--#comp 
#water2.avg_h20_apr2sept, #water2.avg_h20_oct2march, -- #water2
--#horizon.subgroup, #horizon.greatgroup,
--#horizon.max_ec_profile, #horizon.max_sar_profile,
